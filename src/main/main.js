...
// Devuelve el acumulado completo de llamadas: { [cliente]: { [mid]: total } }.
// Lo necesita la vista de Estadísticas para el agregado de todos los clientes.
ipcMain.handle('get-api-usage-all', () => readApiUsage());

// ---------------------------------------------------------------------------
// Estadísticas de uso locales (nunca salen del equipo). Se guardan CONTADORES
// agregados, no eventos: el fichero no crece con el uso, solo con el número de
// acciones distintas que se llegan a usar en cada cliente/BU.
// Fichero usageStats.json:
// { [cliente]: { [mid]: { accesos, primerUso, ultimoUso,
//                         acciones: { [id]: { n, label, ultima } },
//                         vistas:   { [id]: { n, label, ultima } } } } }
// Las acciones (botones) y las vistas (navegación por el menú) se cuentan por separado.
// ---------------------------------------------------------------------------
const USAGE_STATS_FILE = 'usageStats.json';

// Solo se registra lo que ocurre con un cliente activo; el MID siempre acompaña al cliente,
// pero se deja un valor de reserva por si alguna vez llegara vacío.
const NO_MID = '-';

function usageStatsPath() {
    return path.join(app.getPath('userData'), USAGE_STATS_FILE);
}
...
function writeUsageStats(stats) {
    try {
        fs.writeFileSync(usageStatsPath(), JSON.stringify(stats, null, 2));
        return true;
    } catch (e) {
        return false;
    }
...
// Devuelve (creando si hace falta) el nodo de una BU concreta dentro del acumulado.
// Los ficheros de versiones anteriores no tienen 'vistas', por eso se completa aquí.
function getUsageNode(stats, clientName, mid) {
    const bu = mid || NO_MID;
    if (!stats[clientName]) stats[clientName] = {};
    if (!stats[clientName][bu]) {
        stats[clientName][bu] = { accesos: 0, primerUso: null, ultimoUso: null, acciones: {}, vistas: {} };
    }
    const node = stats[clientName][bu];
    node.acciones = node.acciones || {};
    node.vistas = node.vistas || {};
    return node;
}

// Suma un lote de eventos ya agrupados en el renderer. Cada entrada trae el contador
// acumulado desde el último volcado, así que aquí solo se suma. 'type' separa las
// acciones (botones) de las vistas (navegación).
ipcMain.handle('add-usage-events', (event, entries) => {
    if (!Array.isArray(entries) || entries.length === 0) return false;
    const stats = readUsageStats();
    const now = new Date().toISOString();

    entries.forEach(({ clientName, mid, type, action, label, count }) => {
        const n = Number(count) || 0;
        if (!clientName || !action || n <= 0) return;
        const node = getUsageNode(stats, clientName, mid);
        const bucket = type === 'vista' ? node.vistas : node.acciones;
        const current = bucket[action] || { n: 0, label: '', ultima: null };
        current.n += n;
        // La etiqueta se refresca en cada volcado para que siga al texto actual del botón.
        current.label = label || current.label || action;
        current.ultima = now;
        bucket[action] = current;
        node.primerUso = node.primerUso || now;
        node.ultimoUso = now;
    });

    return writeUsageStats(stats);
});

// Registra un acceso a un contexto (cliente + BU).
ipcMain.handle('add-usage-access', (event, clientName, mid) => {
    if (!clientName) return false;
    const stats = readUsageStats();
    const now = new Date().toISOString();
    const node = getUsageNode(stats, clientName, mid);
    node.accesos = (Number(node.accesos) || 0) + 1;
    node.primerUso = node.primerUso || now;
    node.ultimoUso = now;
    return writeUsageStats(stats);
});

// Devuelve el acumulado completo de estadísticas de uso.
ipcMain.handle('get-usage-stats', () => readUsageStats());

// Borra las estadísticas de uso (acciones y accesos). No toca el contador de llamadas API.
